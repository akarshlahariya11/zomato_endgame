...
import { IoFastFoodOutline, IoNutritionOutline, IoThermometerSharp } from "react-icons/io5";
...
const MobileTabs = () => {
  const [allTypes, setAllTypes] = useState([
    {
      ...
      name: "Delivery",
      isActive: false, // isActive is false because it will re-render again and again so it will become true false true false that's why useEffect Hook will toggle it.
    },
    {
      id: "dining",
  ...
  // We can use useEffect to get the pparticular id
  // useEffect(() => {
  //   if (type) {
  //     const updatTypes = allTypes.map((item) => {
  //       if (item.id === type) {
        ...
        {allTypes.map((items) => (
          <Link to={`/${items.id}`}>
            <div
              className={
                type === items.id
                  ? " relative flex flex-col items-center text-xl text-zomato-400  "
